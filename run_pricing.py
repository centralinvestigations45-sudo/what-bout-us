...
CLOTHING = [
    ('Polo Shirt', 'polo-shirt', False), ('Long Sleeve Shirt', 'long-sleeve-shirt', False),
    ('Short Sleeve Shirt', 'short-sleeve-shirt', False), ('Dress Shirt', 'dress-shirt', False),
    ('Casual Pants', 'casual-pants', False), ('Khakis', 'khakis', False), ('Dress Pants', 'dress-pants', False),
...
SHOES = [
    ('Sandals', 'sandals', False), ('Casual Sneakers', 'casual-sneakers', False), ('Dress Shoes', 'dress-shoes', False),
    ('Everyday Socks', 'everyday-socks', False), ('Dress Socks', 'dress-socks', False), ('Ankle Socks', 'ankle-socks', False),
    ('Premium Sneakers', 'premium-sneakers', True), ('Premium Dress Shoes', 'premium-dress-shoes', True), ('Premium Boots', 'premium-boots', True),
]

ACCESSORIES = [
    ('Men’s Watch', 'mens-watch', False), ('Women’s Watch', 'womens-watch', False),
    ('Men’s Bracelet', 'mens-bracelet', False), ('Women’s Bracelet', 'womens-bracelet', False),
    ('Necklace', 'necklace', False), ('Men’s Chain', 'mens-chain', False),
...
def _cards(items, category):
    ...
    for label,key,premium in items:
        ...
        rows.append(f'''<div class="card" style="padding:18px;margin:0"><div>{badge}</div><h3 style="margin:8px 0 4px">{base.esc(label)}</h3><button class="btn {'alt' if premium else ''}" style="margin-top:12px" onclick="equip('{category}','{key}','{base.esc(label)}',{str(premium).lower()})">{button}</button></div>''')
    return ''.join(rows)


def _nav():
    return '<div style="display:flex;gap:10px;flex-wrap:wrap;margin:14px 0 18px"><a class="btn alt" href="/clothing">Clothing</a><a class="btn alt" href="/shoes">Shoes</a><a class="btn alt" href="/accessories">Fashion, Watches & Accessories</a></div>'
...
def shoes_page(): return _catalog_page('Shoes','Pick footwear and socks for your companion, including sandals, sneakers, dress shoes and premium footwear.',SHOES,'shoes')
def accessories_page(): return _catalog_page('Fashion, Watches & Accessories','Pick watches, bracelets, necklaces, chains and rings for your companion.',ACCESSORIES,'accessories')

def styled_companion_page(name):
    html=_original_companion_page(name)
    panel=f'''<div id="wbu-current-look" class="card" style="margin:16px 0;padding:14px 16px"><strong>Current style selections</strong><div id="wbu-style-list" class="sub" style="margin-top:5px">No style items selected.</div><div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:10px"><a href="/accessories" style="text-decoration:underline;color:#fff">Fashion</a><a href="/accessories" style="text-decoration:underline;color:#fff">Watches</a><a href="/shoes" style="text-decoration:underline;color:#fff">Shoes</a><a href="/accessories" style="text-decoration:underline;color:#fff">Accessories</a></div></div><script>(function(){{let k='wbu_style_{name}',x=[];try{{x=JSON.parse(localStorage.getItem(k)||'[]').filter(v=>v.item!=='mens-earrings'&&v.item!=='womens-earrings'&&v.label!=='Men’s Earrings'&&v.label!=='Women’s Earrings');localStorage.setItem(k,JSON.stringify(x))}}catch(e){{}}let el=document.getElementById('wbu-style-list');if(el)el.textContent=x.length?x.map(v=>v.label).join(' · '):'No style items selected.';}})();</script>'''
