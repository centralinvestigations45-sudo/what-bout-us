...
def logo_b64():
    ...
    if not (data.startswith(b'\xff\xd8\xff') and data.endswith(b'\xff\xd9')):
        raise ValueError('Logo is not a complete JPEG')
    return raw


def logo_src():
    return 'data:image/jpeg;base64,' + logo_b64()


def branded_nav():
    return ('<div class="nav"><div class="shell navin"><a class="brand" href="/">'
            '<img src="'+logo_src()+'" alt="What Bout Us™ AI Companions" style="width:72px;height:72px;object-fit:contain;border-radius:12px">'
...
def branded_footer():
    return ('<div class="fine"><img src="'+logo_src()+'" alt="What Bout Us™ AI Companions" style="display:block;width:180px;max-width:55vw;height:auto;margin:0 auto 16px;border-radius:14px">© 2026 What Bout Us<span class="tm">™</span>. All Rights Reserved. · Adults 21+</div>')


def branded_page(title, body):
    html = _original_page(title, body)
    image = PUBLIC_URL + LOGO_PATH + '?v=10'
...
def branded_home():
    html = _original_home()
    hero_logo = '<div style="max-width:1180px;margin:18px auto -8px;padding:0 18px;text-align:center"><img src="'+logo_src()+'" alt="What Bout Us™ AI Companions" style="width:min(420px,86vw);height:auto;border-radius:22px;display:block;margin:auto"></div>'
...
def logo_bytes():
    return base64.b64decode(logo_b64(), validate=True)
