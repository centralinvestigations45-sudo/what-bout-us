...
LOGO_FILE = Path(__file__).resolve().parent / 'static' / 'wbu-logo-v12.jpg'
...
def branded_nav():
    return ('<div class="nav"><div class="shell navin"><a class="brand" href="/">'
            '<img src="'+LOGO_PATH+'?v=12" alt="What Bout Us™ AI Companions" style="width:72px;height:72px;object-fit:contain;border-radius:12px">'
...
def branded_footer():
    return ('<div class="fine"><img src="'+LOGO_PATH+'?v=12" alt="What Bout Us™ AI Companions" style="display:block;width:180px;max-width:55vw;height:auto;margin:0 auto 16px;border-radius:14px">© 2026 What Bout Us<span class="tm">™</span>. All Rights Reserved. · Adults 21+</div>')


def branded_page(title, body):
    html = _original_page(title, body)
    image = PUBLIC_URL + LOGO_PATH + '?v=12'
    meta = ('<meta name="description" content="What Bout Us™ — AI companions with conversation, voice, memory and personalized experiences.">'
            ...
            '<meta property="og:url" content="'+PUBLIC_URL+'/"><meta property="og:image" content="'+image+'"><meta property="og:image:type" content="image/jpeg">'
            ...
            '<link rel="icon" type="image/jpeg" href="'+LOGO_PATH+'?v=12"><link rel="apple-touch-icon" href="'+LOGO_PATH+'?v=12">')
    return html.replace('</head>', meta + '</head>', 1)


def branded_home():
    html = _original_home()
    hero_logo = '<div style="max-width:1180px;margin:18px auto -8px;padding:0 18px;text-align:center"><img src="'+LOGO_PATH+'?v=12" alt="What Bout Us™ AI Companions" style="width:min(520px,92vw);height:auto;border-radius:22px;display:block;margin:auto"></div>'
...
class BrandedHandler(run_topup.TopupHandler):
    def do_GET(self):
        ...
        if path in (LOGO_PATH, '/brand-logo', '/brand-logo.png', '/brand-logo.svg', '/static/wbu-official-logo.webp', '/favicon.ico', '/apple-touch-icon.png'):
            try:
                data = LOGO_FILE.read_bytes()
                if not (data.startswith(b'\xff\xd8\xff') and data.endswith(b'\xff\xd9')):
                    raise ValueError('v12 logo is not a complete JPEG')
