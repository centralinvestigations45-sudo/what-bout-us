...
async function speakReply(text){const status=document.getElementById('status');try{if(currentAudio){currentAudio.pause();currentAudio=null}status.textContent='Simone is speaking...';const r=await fetch('/api/speech',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({text})});if(!r.ok){const e=await r.text();console.log('Speech unavailable',e);status.textContent='Simone\'s voice is temporarily unavailable';return}const blob=await r.blob();if(!blob.size){status.textContent='Simone\'s voice is temporarily unavailable';return}const url=URL.createObjectURL(blob);currentAudio=new Audio(url);currentAudio.playsInline=true;currentAudio.onended=()=>{URL.revokeObjectURL(url);status.textContent='Ready'};try{await currentAudio.play()}catch(e){console.log('Audio playback blocked',e);status.textContent='Tap the screen, then send again'}}catch(e){console.log('Speech error',e);status.textContent='Simone\'s voice is temporarily unavailable'}}
